...
typedef struct DmaCallbackEntry {
    audioapi_dma_callback_t* callback;
    u32 arg0;
    u32 arg1;
} DmaCallbackEntry;
...
RECOMP_CALLBACK(".", AudioApi_InitInternal) void AudioApi_LoadInit() {
    DynDataArr_init(&dmaCallbacks, sizeof(DmaCallbackEntry), DMA_CALLBACK_DEFAULT_CAPACITY);
}
...
RECOMP_EXPORT uintptr_t AudioApi_AddDmaCallback(audioapi_dma_callback_t* callback, u32 arg0, u32 arg1) {
    u16 id = dmaCallbacks.count;
    DmaCallbackEntry entry = { callback, arg0, arg1 };
...
s32 AudioApi_Dma_Callback(uintptr_t devAddr, void* ramAddr, size_t size, size_t offset) {
    ...
    DmaCallbackEntry* entry = DynDataArr_get(&dmaCallbacks, id);
    return entry->callback(ramAddr, size, offset, entry->arg0, entry->arg1);
}
...
RECOMP_PATCH void* AudioLoad_DmaSampleData(uintptr_t devAddr, size_t size, s32 arg2, u8* dmaIndexRef, s32 medium) {
    ...
    if (IS_KSEG0(devAddr)) {
        ramAddr = AudioApi_RspCacheAlloc((void*)devAddr, size, &didAllocate);
        if (!ramAddr) {
            return NULL;
        }
        if (didAllocate) {
            AudioApi_Dma_Mod(devAddr, ramAddr, size);
        }
        return ramAddr;
    }

    if (IS_DMA_CALLBACK_DEV_ADDR(devAddr)) {
        ramAddr = AudioApi_RspCacheAlloc(NULL, size * SAMPLE_SIZE, &didAllocate);
        if (!ramAddr) {
            return NULL;
        }
        AudioApi_Dma_Callback(devAddr, ramAddr, size, arg2);
        return ramAddr;
    }
